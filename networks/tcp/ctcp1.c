...
void ctcp_receive(ctcp_state_t *state, ctcp_segment_t *segment, size_t len) {
  ...
  if (len - SEG_HEADER_SIZE != 0) {
    ...
    ll_node_t *node_rw_first = ll_front(state->rw->segments);
    bool inside_rw = 1;
    if(node_rw_first!=NULL){
      ctcp_segment_t *segment_rw_first = node_rw_first->object;
      if(ntohl(segment_rw_first->seqno) + state->rw->max_size
          <= ntohl(segment->seqno) - ntohs(segment->len)){
            inside_rw = 0;
          }
    }

    if(ntohl(segment->seqno) > state->rw->ackno &&
      ntohs(segment->len) - SEG_HEADER_SIZE <= state->rw->max_size-state->rw->size &&
      inside_rw){
      ...
      while(node->next!=NULL){
        next_seg = node->next->object;
        if(ntohl(segment->seqno) < next_seg->ackno){
  ...
  /* 6. Received ACK or force_ackbacked ACK from data segments */
